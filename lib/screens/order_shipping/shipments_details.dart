...
class _OrderShipmentsScreenState extends State<OrderShipmentsScreen> {
  ...
  void initState() {
    ...
    orderShipmentsBloc?.add(OrderShipmentsFetchDataEvent(widget.orderDetailsModel?.id ?? 0));
    super.initState();
  }
    ...
    return BlocConsumer<OrderShipmentsBloc, OrderShipmentsBaseState>(
      ...
      builder: (BuildContext context,OrderShipmentsBaseState state) {
        ...
        if (state is OrderShipmentsListDataState) {
          ...
          if (state.status == OrderShipmentsStatus.fail) {
    ...
    return shipmentModel?.map((ViewShipments? shipments){
      return Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("${StringConstants.trackingNumber.localized()} ${shipments?.trackNumber}", style: Theme.of(context).textTheme.titleLarge?.copyWith(
                fontWeight: FontWeight.bold
            )),
            const SizedBox(height: AppSizes.spacingSmall,),
            Text("${StringConstants.shipment.localized()} #${shipments?.id}", style: Theme.of(context).textTheme.titleLarge?.copyWith(
                fontWeight: FontWeight.bold
            )),
            const Divider(),
            ListView.separated(itemBuilder: (context, index){
              return Column(
                children: [
                  childData(StringConstants.sku, shipments?.items?[index].sku ?? "", context),
                  childData(StringConstants.name, shipments?.items?[index].name ?? "", context),
                  childData(StringConstants.qty, shipments?.items?[index].qty.toString() ?? "", context),
              ]
              );
